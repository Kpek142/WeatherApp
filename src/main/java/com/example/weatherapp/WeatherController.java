...
public class WeatherController {
    ...
    private void fetchCitySuggestions(String query) {
        ...
        httpClient.sendAsync(request, HttpResponse.BodyHandlers.ofString())
                ...
                .thenAccept(response -> Platform.runLater(() -> {
                    ...
                    if (!suggestionsList.getItems().isEmpty()) showSuggestions();
                    else hideSuggestions();
                }));
    }

    private void getWeather(double lat, double lon, String cityName) {
        String url = String.format("https://api.openweathermap.org/data/2.5/weather?lat=%f&lon=%f&appid=%s&units=metric&lang=ru",
                lat, lon, API_KEY);
        HttpRequest request = HttpRequest.newBuilder().uri(URI.create(url)).build();
    ...
    private void initDatabase() {
        try (Connection conn = DriverManager.getConnection("jdbc:sqlite:weather.db")) {
            conn.createStatement().execute("CREATE TABLE IF NOT EXISTS last_city (id INTEGER PRIMARY KEY, name TEXT, lat REAL, lon REAL)");
        } catch (SQLException e) { e.printStackTrace(); }
    }

    private void saveLastCity(String name, double lat, double lon) {
        try (Connection conn = DriverManager.getConnection("jdbc:sqlite:weather.db");
             PreparedStatement pstmt = conn.prepareStatement("INSERT OR REPLACE INTO last_city(id, name, lat, lon) VALUES(1, ?, ?, ?)")) {
    ...
    private void loadLastCity() {
        try (Connection conn = DriverManager.getConnection("jdbc:sqlite:weather.db");
             ResultSet rs = conn.createStatement().executeQuery("SELECT * FROM last_city WHERE id = 1")) {
            if (rs.next()) getWeather(rs.getDouble("lat"), rs.getDouble("lon"), rs.getString("name"));
        } catch (SQLException e) { e.printStackTrace(); }
    }
}